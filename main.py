...
class AI2SQLService:
    ...
    def query(self, question: str, collect_logs: bool = False) -> Dict:
        """
        处理单个查询问题
        
        Args:
            question: 自然语言问题
            collect_logs: 是否收集日志到结果中
    ...
    def query_with_retries(self, question: str, collect_logs: bool = False,
        ...
        for attempt in range(1, max_retries + 1):
            ...
            logger.info(f"\n=== 第 {attempt} 次尝试执行查询 ===")
            result = self.query(question, collect_logs=collect_logs)
            last_result = result

            # 如果本轮查询失败（包括大模型错误、SQL 生成错误、数据库错误等），且还有重试机会，继续重试
            if not result.get("success"):
                if attempt < max_retries:
                    logger.warning("本次查询处理失败，将重新尝试...")
                continue
    ...
    def run_interactive(self):
        """运行交互式循环，持续问询直到用户退出"""
        logger.info("=" * 60)
        logger.info("AI转SQL服务已启动")
        logger.info("输入问题开始查询，输入 'quit' 或 'exit' 退出")
        logger.info("=" * 60)
