...
- 以下为大桥局人员信息表的 字段与扩展说明（帮助你理解表结构）：
    ...
    - 岗位是类似GW-20250116-0021编号，职位才是类似单元长的中文，如果用岗位为单元长，那就默认岗位用职位查找，反之一样
    - 人数就是大桥局人员信息表的档案编号个数
    - 其余就是表的字段对应查找
- 以下为班前讲话班组字典表与责任单元字典表的 字段与扩展说明（帮助你理解表结构）：
    - 责任单元字典表.责任单元名称(如东山跨海特大桥420#-491#平台搭设及下构施工),责任单元字典表.单元长姓名,责任单元字典表.工程类型名称,责任单元字典表.状态(施工/停工)分别对应的班组负责的单元地方/部位，该单元负责人，该单元工程类型，该单元是施工还是停工
    - 责任单元字典.档案编号 与大桥局人员信息表.档案编号 对应，可通过责任单元字典.档案编号查询 通过档案编号可以在大桥局人员信息表查询该单元负责人基本信息
    - 班前讲话班组字典.班组，班前讲话班组字典.班组长，班前讲话班组字典.手机号，班前讲话班组字典.班组成员，班前讲话班组字典.公司，班前讲话班组字典.所属项目分别对应着该班组的具体班组名称，班组长名称，班组长的手机号，班组成员，班组的公司，班组所属项目
    - 班前讲话班组字典表.安全责任单元 与 责任单元字典.ID 对应
    - 责任单元字典表是负责的地方，部位，以及该地方的负责人和状态。
    - 班前讲话班组字典表 就是用来查看班组对应的一些信息，如班组的具体班组名称，班组长名称，班组长的手机号，班组成员，班组的公司，班组所属项目，班组负责的地方，部位，以及该地方的负责人和状态(通过责任单元字典表)。
    - 班前讲话班组字典表.班组ID 与 跟班作业记录表.班组 对应 ，可通过跟班作业记录表.班组查询到班前讲话班组字典表.班组ID，从而找到跟班作业班组对象的具体信息
- “某人带班了什么 / 最近带了哪些班？” → 使用 M1：按姓名查询带班作业记录表，返回【带班日期、带班作业工序及地点】，按时间倒序，LIMIT<=50。
