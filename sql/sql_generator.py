...
3. 输出严格 JSON，可两种格式：
   - 模板：{{"template_id": "M1", "params": {{"person_name": "王飞"}}}}
   - 自由生成：{{"template_id": "free", "sql": "SELECT ..."}}
...
**重要**：自由生成SQL时，请尽可能选择更多相关字段，输出尽可能多的信息。优先选择业务相关的字段（如姓名、日期、地点、工作内容、状态等），避免只选择ID等无意义的字段。
**JOIN使用规范**：优先使用INNER JOIN而非LEFT JOIN；如果必须使用LEFT JOIN，需要在WHERE子句中过滤掉NULL值（如 `关联表.关键字段 IS NOT NULL`）。
...
- 如果问题超出以上典型场景，且你能根据 schema 自己写出安全的 SQL，可以使用 template_id="free" 并直接返回 sql。
 ...
 - 使用模板（M1~M6）：
  {{"template_id": "M1", "params": {{"person_name": "王飞"}}}}
  {{"template_id": "M2", "params": {{"person_name": "罗康康"}}}}
  {{"template_id": "M5", "params": {{"team_name": "武汉化工"}}}}
  {{"template_id": "M6", "params": {{"date": "2025-04-01", "unit_name": "对门山隧道"}}}}
- 自由生成：
  {{"template_id": "free", "sql": "SELECT 字段1, 字段2, 字段3, ... FROM 表名 WHERE ... ORDER BY ..."}}
"""
...
问：王飞最近跟班了哪些工序？
{{"template_id": "M2", "params": {{"person_name": "王飞"}}}}
说明：王飞是人名，使用M2
...
问：第一班组的跟班作业？
{{"template_id": "M5", "params": {{"team_name": "第一班组"}}}}
说明：第一班组是班组名称，使用M5

问：王飞最近做了什么（带班和跟班都要）？
{{"template_id": "M3", "params": {{"person_name": "王飞"}}}}
说明：王飞是人名，使用M3
...
"""
    ...
    def extract_template_and_params(self, llm_output: str) -> Tuple[str, Dict, str]:
        ...
        if fixed_json and fixed_json != original_json_str:
            try:
                ...
                logger.debug(f"[DEBUG] 方法3失败: {e}")

        # 方法4: 如果所有JSON解析都失败，使用简单的正则表达式提取关键参数（作为最后手段）
        logger.debug(f"[DEBUG] 所有JSON解析方法都失败，使用正则表达式备用方法")
    ...
    def _extract_from_dict(self, data: Dict) -> Tuple[str, Dict, str]:
        ...
        try:
            ...
            logger.debug(f"[DEBUG] _extract_from_dict 异常: {type(e).__name__}: {e}")
    ...
    def extract_sql(self, llm_output: str) -> str:
        ...
        try:
            ...
            logger.warning(f"警告：模板化解析失败，回退到直接提取SQL模式：{e}")
